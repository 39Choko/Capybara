...
public class CapybaraEntity extends TameableEntity implements GeoEntity {
   ...
   private PlayState predicate(AnimationState<?> state) {
      if (state.isMoving()) {
         state.getController()
               .setAnimation(RawAnimation.begin().then("animation.capybara.walk", Animation.LoopType.LOOP));
         return PlayState.CONTINUE;
      }

      if (this.isInSittingPose()) {
         state.getController().setAnimation(
               RawAnimation.begin().then("animation.capybara.seat", Animation.LoopType.HOLD_ON_LAST_FRAME));
         return PlayState.CONTINUE;
      }
